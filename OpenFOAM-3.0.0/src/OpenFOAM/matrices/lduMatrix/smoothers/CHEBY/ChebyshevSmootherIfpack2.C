...
) const
{
    ...
    if(eigFirstTimeComputed_)
    {
        ...
        // else
        {
            // Timer::startTimer("Cheby PCGs Amul");
            matrix_.Amul(w, x, interfaceBouCoeffs_, interfaces_, cmpt);
            // Timer::endTimer("Cheby PCGs Amul");
        ...
        for(label nIter=0; nIter<nPCGsCheby_; nIter++)
        {
            ...
            // Timer::startTimer("PCG precondition");
            // Apply preconditioner
            diagPre.precondition(w, r, cmpt);
            // Timer::endTimer("PCG precondition");

            // Timer::startTimer("Cheby PCGs gSumProd");
            // Update rho:
            rho = gSumProd(w, r);
            // Timer::endTimer("Cheby PCGs gSumProd");
        ...
        // Timer::startTimer("Cheby PCGs computeMaxEig");
        computeMaxEig(alphas, betas, nPCGsCheby_, 1);
        // Timer::endTimer("Cheby PCGs computeMaxEig");

        // Timer::endTimer("Cheby PCGs");
    ...
    else
    {
        ...
        // At sweep = 0, update x
        // Timer::startTimer("Cheby Precondition");
        diagPre.precondition(w, r, cmpt);
        // Timer::endTimer("Cheby Precondition");
