...
void interpolate1_(struct Parameter* paramt)
{
	...
	double* weiPtr = paramt -> weiPtr;
	double* _1wPtr = paramt -> _1wPtr;
	int* ownPtr = paramt -> ownPtr;
	int* neiPtr = paramt -> neiPtr;
	const struct rowSubsection** subSecPtr = paramt -> subSecPtr;
	int secNum = paramt -> secNum;
	int typeSize = paramt -> typeSize;
	int eleNum = paramt -> eleNum;
	double* usage = paramt -> usage;

	int myId, myNF, myStart, lDblVal, lIntVal;
	volatile int reply;
	struct rowSubsection subSec;
	int i,j,k,l,m,n,start,count;
	int NF, NC, NFN, EM, totalSize;
	...
	for( n=0; n<secNum; n++)
	{
		...
		lDblVal = myNF * sizeof(double);
		lIntVal = myNF * sizeof(int);
		NF = myNF;
		int owner[NF], neighbor[NF];
		reply = 0;
		...
		//模板形参中元素的个数
		EM = eleNum;
		totalSize = (NF + NC + NFN) * EM * sizeof(double) //face value, cell value by owner and by neighbor
				  + 2 * NF * sizeof(double)               //owner and neighbor
				  + 2 * NF * sizeof(int);                 //weight and 1-weght
		...
		double faceVal[NF*EM], cellValOw[NC*EM], cellValNe[NFN*EM];
		double weight[NF], _1weight[NF];
		...
		//计算
		double  faceVal_, cellValOw_, cellValNe_, weight_, _1weight_;
		double *cvOwPtr, *cvNePtr, *faVaPtr;
		int colNum, localN, _2j;
		for (i = 0; i < myNF; i++)
		{
